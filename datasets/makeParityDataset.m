...
%       +1 if its label contains an even number of 1
%       -1 otherwise
function makeExtendeXorDataset(file)
% Create a dataset for the odd-even recognition problem
% USAGE: makeExtendeXorDataset(<file.config>)
...
try
    if nargin ~= 1
        file='ParityDataset.config';     %default config file
    end
    ...
    % graphDim nodeLabelsDim graphDensity rejectUpperThreshold rejectLowerThreshold 
    % trainSet.graphNum validationSet.graphNum testSet.graphNum
    params_name={'graphDim' 'nodeLabelsDim' 'graphDensity' 'rejectUpperThreshold' 'rejectLowerThreshold'...
                 'trainSet.graphNum' 'validationSet.graphNum' 'testSet.graphNum'};
...
for i=1:tmp.nNodes
    tmp.targets(i)=mod(sum(tmp.nodeLabels(:,i)),2)*2-1;
end
...
for i=1:size(params,2)
    if params(i)==0
        try
            err(0,['makeExtendeXorDataset: No parameter <' char(params_name(i)) '> in the configuration file']);
        catch
            ok=0;
        end
    end
